...
int student_vector_add(struct student_vector *student_vector, struct student *student)
{
    if(student_vector->capacity == student_vector->count) {
        ...
        free(student_vector->students); // student_vector va pointa acum la new_students                
...
int student_vector_remove(struct student_vector *student_vector, int index)
{
    ...
    free(student_vector->students[index]->name); // added 
    free(student_vector->students[index]->surname); //added
    student_vector->students[index]->name=NULL;
    student_vector->students[index]->surname=NULL;
    free(student_vector->students[index]);//added
    student_vector->students[index]=NULL;
...
int main()
{
    ...
    for(;;) {
        ...
        if('e' == command[0] && 'x' == command[1] && 'i' == command[2] && 't' == command[3] && '\x00' == command[4]) {
                ...
                free(command);  // added
                for (i = 0; i < student_vector->count; i += 1) {
                    free(student_vector->students[i]->name); // added 
                    free(student_vector->students[i]->surname); // added
                    student_vector->students[i]->name=NULL;
                    student_vector->students[i]->surname=NULL;
                    free(student_vector->students[i]);
                    student_vector->students[i]= NULL;
                }
                free(student_vector->students); // added
                student_vector->students=NULL;
                free(student_vector); // added
                student_vector=NULL;
            return 0;
        }
        // list
        ...
        // add
            ...
            if(NULL == new_student->name || NULL == new_student->surname) {
                ...
                if(new_student->name!=NULL)
                    free(new_student->name); // added
                if(new_student->surname!=NULL)
                    free(new_student->surname); // added
                new_student->surname=NULL;
                new_student->name=NULL;
                free(new_student); // added
                new_student=NULL;
                continue;
            }
